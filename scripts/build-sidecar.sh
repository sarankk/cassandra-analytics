...
# The version the locally built artifacts are published under. This is deliberately NOT the
# `sidecarVersion` from gradle.properties: the Artifactory/Maven Central repositories are declared
# before the `dependencies` mavenLocal repository, so publishing under a version that also exists
# remotely means the remote artifact silently wins and the local build is ignored. Use a version
# that only ever exists locally, and select it explicitly with -PsidecarVersion.
SIDECAR_BUILD_VERSION="${SIDECAR_BUILD_VERSION:-1.0.1-analytics}"

# Build from a local sidecar checkout. For instance,
# LOCAL_SIDECAR_REPO=/PATH/TO/cassandra-sidecar ./scripts/build-sidecar.sh
if [ "x${LOCAL_SIDECAR_REPO}" != "x" ]; then
  if [ ! -x "${LOCAL_SIDECAR_REPO}/gradlew" ]; then
    echo "LOCAL_SIDECAR_REPO=${LOCAL_SIDECAR_REPO} does not look like a cassandra-sidecar checkout (no executable gradlew)" >&2
    exit 1
  fi
  ...
  # check out the correct sidecar version:
  # if the SIDECAR_BRANCH directory does not exist; we initialize from scratch
...
set +x
echo
echo "Published Cassandra Sidecar ${SIDECAR_BUILD_VERSION} to ${SIDECAR_JAR_DIR}"
echo "Build and test analytics against it by passing the version explicitly, e.g.:"
echo
echo "  ./gradlew -PsidecarVersion=${SIDECAR_BUILD_VERSION} :cassandra-analytics-integration-tests:test"
echo
echo "The dependencies directory is a local file repository, so Gradle references these artifacts in"
echo "place rather than copying them into ~/.gradle/caches. Re-running this script at the SAME version"
echo "is therefore picked up on the next build with no --refresh-dependencies and no version bump"
echo "(verified: replacing a jar in place and re-resolving returns the new content). Pass"
echo "--refresh-dependencies only if the set of dependency coordinates changed, not merely their bytes."
echo
