...
class SSTableWriterImplementationTest
{
    ...
    void testSSTableWriterConfiguration() throws NoSuchFieldException, IllegalAccessException
    {
        CQLSSTableWriter.Builder builder = SSTableWriterImplementation.configureBuilder(writeDirectory.getAbsolutePath(),
                                                                                        ...
                                                                                        sstables -> {},
                                                                                        // Cassandra 6.0 makes the partitioner constructors non-public
    ...
    void testGetProducedSSTables() throws IOException
    {
        ...
        SSTableWriterImplementation writer = new SSTableWriterImplementation(writeDirectory.getAbsolutePath(),
                                                                             // required in order to insert ordered ints
        ...
        // Read the format and the first generation rather than assume them: the selected format and the static
        // sstable id of AbstractSSTableSimpleWriter both carry over from whatever ran earlier in this JVM
        ...
        assertThat(produced.stream().map(e -> e.baseFilename)).containsExactlyInAnyOrder(
            ...
            toSStableFileName(sstableFormat, sstableVersion, firstGeneration + 1));
        ...
        assertThat(produced.stream().map(e -> e.baseFilename))
            .containsExactly(toSStableFileName(sstableFormat, sstableVersion, firstGeneration + 2));
        ...
        assertThat(produced.size()).isEqualTo(1);
        assertThat(produced.stream().map(e -> e.baseFilename))
            .containsExactly(toSStableFileName(sstableFormat, sstableVersion, firstGeneration + 3));
    }

    // A base filename is version-generation-format, for example pa-2-big
    private static int lowestGeneration(Set<SSTableDescriptor> produced)
    {
        return produced.stream()
                       .mapToInt(descriptor -> Integer.parseInt(descriptor.baseFilename.split("-")[1]))
                       .min()
                       .orElseThrow(() -> new AssertionError("No sstable was produced"));
    }
