...
public abstract class AbstractCqlList extends CqlCollection implements CqlField.CqlList
{
        ...
        for (Object o : (List<?>) value)
        {
            if (ttl != NO_TTL)
            {
                rowBuilder.addCell(CqlType.expiring(cd, timestamp, ttl, now, type().serialize(o),
                                                    CellPath.create(ByteBuffer.wrap(TimeUUID.Generator.nextTimeUUIDAsBytes()))));
            }
            else
            {
